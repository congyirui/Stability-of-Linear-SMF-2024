...
load('experiment_os_20240218') % The data is too large. One can run "ExampleB_1_OS" to derive the same data.
...
% for selected_trial = 1: 30
%     for selected_p_num = 3: 3
        figure,
        subplot(2, 1, 1)
        ...
        % selected_trial = 2;
        % selected_p_num = 3;
        plot(k_sequence_drawing, diameters_inf(:, selected_trial, selected_p_num), '-.', 'LineWidth', 1.2)
        k_reset = highlights{selected_trial, selected_p_num}.k_reset;
        indices_k_reset_draw = find(k_reset == 1);
        plot(k_sequence_drawing(indices_k_reset_draw), diameters_inf(indices_k_reset_draw, selected_trial, selected_p_num), '*', 'LineWidth', 1.2)
        ...
        ylabel('Diameter')
        ylim([0 60])
%         title([num2str(selected_trial) ', ' num2str(selected_p_num)])
        grid on;
        ...
        plot(k_sequence_drawing, bounds_estimation_gap_inf(:, selected_trial, selected_p_num), '-.', 'LineWidth', 1.2)
        plot(k_sequence_drawing(indices_k_reset_draw), bounds_estimation_gap_inf(indices_k_reset_draw, selected_trial, selected_p_num), '*', 'LineWidth', 1.2)
...
for i = 1: n
    ...
    for k = k_sequence_drawing
        interval_max(k+kIndexC) = highlights{selected_trial, selected_p_num}.IH_posterior_OITCZSMF_real{k+kIndexC}.c(i) + highlights{selected_trial, selected_p_num}.IH_posterior_OITCZSMF_real{k+kIndexC}.cwb(i);
        interval_min(k+kIndexC) = highlights{selected_trial, selected_p_num}.IH_posterior_OITCZSMF_real{k+kIndexC}.c(i) - highlights{selected_trial, selected_p_num}.IH_posterior_OITCZSMF_real{k+kIndexC}.cwb(i);
        interval_center(k+kIndexC) = highlights{selected_trial, selected_p_num}.IH_posterior_OITCZSMF_real{k+kIndexC}.c(i);
    end
    
    figure,
    plot(k_sequence_drawing, interval_max, '--', k_sequence_drawing, interval_min, '--', k_sequence_drawing, interval_center, k_sequence_drawing, highlights{selected_trial, selected_p_num}.x_sequence(i, :), 'LineWidth', 1.2)
    set(gca,'FontSize',12);
